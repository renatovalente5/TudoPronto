...
export const ETIQUETAS_DONO = [
  'Instruções claras', 'Pagou a horas', 'Casa em bom estado', 'Boa comunicacao',
...
export function rotasSocial (api) {
  ...
  api.post('/v1/servicos/:id/avaliar', async ({ env, pedido, params, corpo }) => {
    ...
    if (s.estado !== 'concluido') erro(400, 'Só se avalia depois de a limpeza estar concluída.')

    const avaliadoId = papel === 'dono' ? s.profissional_id : s.dono_id
    if (!avaliadoId) erro(400, 'Não há ninguem para avaliar nesta limpeza.')

    const ja = await env.BD.prepare(
      'SELECT id FROM avaliacoes WHERE servico_id = ?1 AND autor_id = ?2'
    ).bind(s.id, c.id).first()
    if (ja) erro(409, 'Já avaliou esta limpeza.')
    ...
    if (outra) {
      ...
      await avisar(env, avaliadoId, {
        ...
        titulo: 'As avaliações já estão visiveis',
      ...
      await avisar(env, avaliadoId, {
        ...
        corpo: 'A avaliação aparece assim que avaliar também — ou daqui a 14 dias.',
        ligacao: `/app/#/servico/${s.id}`,
      })
    }
  ...
  api.get('/v1/servicos/:id/avaliacoes', async ({ env, pedido, params }) => {
    ...
    const s = await env.BD.prepare('SELECT dono_id, profissional_id FROM servicos WHERE id = ?1')
      .bind(params.id).first()
    if (!s) erro(404, 'Essa limpeza já não existe.')
    if (s.dono_id !== c.id && s.profissional_id !== c.id) erro(403, 'Essa limpeza não é sua.')
  ...
  api.get('/v1/perfil/:id', async ({ env, pedido, params }) => {
    ...
    if (!p) erro(404, 'Essa pessoa já não está no Tudo Pronto.')
  ...
  api.post('/v1/equipa/convidar', async ({ env, pedido, corpo }) => {
    ...
    if (profissionalId) {
      ...
      if (ja) erro(409, 'Essa pessoa já esta na sua equipa.')
    }
    ...
    if (profissionalId) {
      await avisar(env, profissionalId, {
        ...
        corpo: 'Vai passar a receber as limpezas deste anfitrião.',
        ligacao: '/app/#/equipa',
      })
    } else if (endereco) {
      await enviar(env, {
  ...
  api.post('/v1/equipa/entrar', async ({ env, pedido, corpo }) => {
    ...
    if (!e) erro(404, 'Esse código não serve. Peça um novo a quem a convidou.')
    if (e.dono_id === c.id) erro(400, 'Não pode juntar-se a sua própria equipa.')
    await env.BD.prepare(
    ...
    await avisar(env, e.dono_id, {
      ...
      corpo: 'Já lhe pode entregar limpezas directamente.',
      ligacao: '/app/#/equipa',
    })
  ...
  api.del('/v1/equipa/:id', async ({ env, pedido, params }) => {
    ...
    if (!e) erro(404, 'Esse vinculo já não existe.')
    // Qualquer um dos dois pode desfazer: ninguem fica preso a equipa de ninguem.
    if (e.dono_id !== c.id && e.profissional_id !== c.id) erro(403, 'Esse vinculo não é seu.')
  ...
  api.get('/v1/servicos/:id/mensagens', async ({ env, pedido, params }) => {
    ...
    const s = await env.BD.prepare('SELECT dono_id, profissional_id FROM servicos WHERE id = ?1')
      .bind(params.id).first()
    if (!s) erro(404, 'Essa limpeza já não existe.')
    if (s.dono_id !== c.id && s.profissional_id !== c.id) erro(403, 'Essa conversa não é sua.')
    const r = await env.BD.prepare(`
  ...
  api.post('/v1/servicos/:id/mensagens', async ({ env, pedido, params, corpo }) => {
    ...
    if (!s) erro(404, 'Essa limpeza já não existe.')
    if (s.dono_id !== c.id && s.profissional_id !== c.id) erro(403, 'Essa conversa não é sua.')
    if (!s.profissional_id) erro(400, 'Ainda não há com quem falar nesta limpeza.')
  ...
  api.post('/v1/denuncias', async ({ env, pedido, corpo }) => {
    ...
    await env.BD.prepare(`
      ...
      AGORA()).run()
    return json({ id, mensagem: 'Recebemos a sua denúncia. Vamos analisa-la.' }, 201)
  })
}
